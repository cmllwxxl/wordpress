...
import { ArrowLeft, RefreshCw, CheckCircle2, XCircle, Clock, Globe, AlertTriangle, Play, Pause, Shield, ShieldAlert, ShieldCheck, ShieldX } from 'lucide-react';
...
export default function UptimeMonitorPage() {
    ...
    const checkSite = async (siteId: string, url: string) => {
        ...
        try {
            ...
            const { status, latency, code, error, ssl } = response.data;
    ...
    // SSL Stats
    const sslExpiringCount = Object.values(results).filter(r =>
        r.ssl && r.ssl.valid && r.ssl.daysRemaining !== undefined && r.ssl.daysRemaining <= 30
    ).length;
    const sslExpiredCount = Object.values(results).filter(r =>
        r.ssl && (!r.ssl.valid || (r.ssl.daysRemaining !== undefined && r.ssl.daysRemaining <= 0))
    ).length;
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-8">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-medium text-zinc-500">SSL证书</h3>
                            {sslExpiredCount > 0 ? (
                        ...
                        <p className="text-3xl font-bold text-zinc-900 dark:text-white">
                            {sslExpiredCount > 0 ? (
                                <><span className="text-red-500">{sslExpiredCount}</span><span className="text-sm text-zinc-400 font-normal ml-1">已过期</span></>
                            ) : sslExpiringCount > 0 ? (
                                <><span className="text-amber-500">{sslExpiringCount}</span><span className="text-sm text-zinc-400 font-normal ml-1">即将过期</span></>
                            ) : (
                                <><span className="text-green-500">✓</span><span className="text-sm text-zinc-400 font-normal ml-1">全部正常</span></>
                            )}
                        </p>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
                                <tr>
                                    <th className="px-6 py-4 font-medium text-zinc-500 w-[25%]">站点名称</th>
                                    <th className="px-6 py-4 font-medium text-zinc-500 w-[10%]">状态</th>
                                    <th className="px-6 py-4 font-medium text-zinc-500 w-[12%]">响应时间</th>
                                    <th className="px-6 py-4 font-medium text-zinc-500 w-[8%]">状态码</th>
                                    <th className="px-6 py-4 font-medium text-zinc-500 w-[20%]">SSL证书</th>
                                        ...
                                        <tr key={site.id} className="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                {isPending ? (
                                                    ...
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400" title={result.ssl.error}>
                                                        <ShieldX className="w-3 h-3" />
                                                        检测失败
                                                    </span>
                                                ) : !result.ssl.valid || (result.ssl.daysRemaining !== undefined && result.ssl.daysRemaining <= 0) ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400" title={`过期于: ${result.ssl.validTo ? new Date(result.ssl.validTo).toLocaleDateString() : 'Unknown'}`}>
                                                        <ShieldX className="w-3 h-3" />
                                                        已过期
                                                    </span>
                                                ) : result.ssl.daysRemaining !== undefined && result.ssl.daysRemaining <= 30 ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400" title={`过期于: ${result.ssl.validTo ? new Date(result.ssl.validTo).toLocaleDateString() : 'Unknown'}`}>
                                                        <ShieldAlert className="w-3 h-3" />
                                                        {result.ssl.daysRemaining}天后过期
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400" title={`过期于: ${result.ssl.validTo ? new Date(result.ssl.validTo).toLocaleDateString() : 'Unknown'}\n颁发者: ${result.ssl.issuer || 'Unknown'}`}>
                                                        <ShieldCheck className="w-3 h-3" />
                                                        {result.ssl.daysRemaining}天
                                                    </span>
                                                )}
                                            </td>
